<?php
require_once "Role2.php";
require_once "PrivilegedUser2.php";

session_start();

if (isset($_SESSION['admin'])) {
    $u = PrivilegedUser::getByUsername($_SESSION['admin']);
}
/*
if (!($u->hasPrivilege("Add_facA")))
{
	...
	if($r)
	{
		  $q1=mysqli_query($conn,"select * from faculty where email='$email' and programme='$prg' and semester='$sem'");
			...
			else
			{
				...
				mysqli_query($conn,"insert into faculty values('','$user_name','$name','$Designation','$prg','$sem','$email','$pas','$mob',now(),'0','$course','$code')");

				$err="<font color='blue' size='5px'><center>New Faculty Successfully Added.</center></font>";

				$teacher_id_query = "select id from faculty where email='$email'";
				$teacher_id_query_result = mysqli_query($conn,$teacher_id_query) or die(mysqli_error($conn));

				$row = mysqli_fetch_array($teacher_id_query_result);

				$ans = $row['id'];
				$insert_query = "insert into user_role1(id1,role_id) values('$ans', '2')";
